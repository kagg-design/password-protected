<?php
/**
 * Password_Protected_Admin class file.
 *
...
class Password_Protected_Admin {
	 ...
	 * @var string
	 */
	public $options_group = 'password-protected';
	...
	public function __construct( $plugin ) {
		...
		add_action( 'admin_init', [ $this, 'password_protected_settings' ], 5 );
		add_action( 'admin_init', [ $this, 'add_privacy_policy' ] );
		add_action( 'admin_menu', [ $this, 'admin_menu' ] );
		add_action( 'password_protected_help_tabs', [ $this, 'help_tabs' ], 5 );
		add_action( 'admin_notices', [ $this, 'password_protected_admin_notices' ] );
		add_filter( 'plugin_row_meta', [ $this, 'plugin_row_meta' ], 10, 4 );
		add_filter( 'plugin_action_links_password-protected/password-protected.php', [ $this, 'plugin_action_links' ] );
		add_filter(
			'pre_update_option_password_protected_password',
			[ $this, 'pre_update_option_password_protected_password' ],
			10,
			2
		);
	...
	public function add_privacy_policy() {
		...
		$content = _x(
			'The Password Protected plugin stores a cookie on successful password login containing a hashed version of the entered password. It does not store any information about the user. The cookie stored is named <code>bid_n_password_protected_auth</code> where <code>n</code> is the blog ID in a multisite network',
			'privacy policy content',
			'password-protected'
		);

		wp_add_privacy_policy_content(
			__( 'Password Protected Plugin', 'password-protected' ),
	...
	public function admin_menu() {
		...
		add_action( 'load-' . $this->settings_page_id, [ $this, 'add_help_tabs' ], 20 );
	...
	public function password_protected_settings() {
		...
		add_settings_section(
			'password_protected',
			'',
			[ $this, 'password_protected_settings_section' ],
			$this->options_group
		);
		...
		add_settings_field(
			...
			[ $this, 'password_protected_status_field' ],
			$this->options_group,
			'password_protected'
		);
		...
		add_settings_field(
			...
			[ $this, 'password_protected_permissions_field' ],
			$this->options_group,
			'password_protected'
		);
		...
		add_settings_field(
			...
			[ $this, 'password_protected_password_field' ],
			$this->options_group,
			'password_protected'
		);
		...
		if ( ! $this->plugin->is_vip_env() ) {
			add_settings_field(
				...
				[ $this, 'password_protected_allowed_ip_addresses_field' ],
				$this->options_group,
				'password_protected'
			);
		}
		...
		add_settings_field(
			...
			[ $this, 'password_protected_remember_me_field' ],
			$this->options_group,
			'password_protected'
		);
		...
		add_settings_field(
			...
			[ $this, 'password_protected_remember_me_lifetime_field' ],
			$this->options_group,
			'password_protected'
		);
		...
		register_setting(
			$this->options_group,
			'password_protected_password',
			[ $this, 'sanitize_password_protected_password' ]
		);
		...
		if ( is_array( $val ) ) {
			if ( empty( $val['new'] ) ) {
				...
				add_settings_error(
					'password_protected_password',
					'password_protected_password',
					__( 'New password not saved. When setting a new password please enter it in both fields.', 'password-protected' )
				);
				...
				add_settings_error(
					'password_protected_password',
					'password_protected_password',
					__( 'New password not saved. Password fields did not match.', 'password-protected' )
				);
				...
				add_settings_error(
					'password_protected_password',
					'password_protected_password',
					__( 'New password saved.', 'password-protected' ),
					'updated'
				);
	...
	public function sanitize_ip_addresses( $val ) {
		...
		$ip_addresses = array_map( [ $this, 'validate_ip_address' ], $ip_addresses );
	...
	public function password_protected_settings_section() {
		...
		echo '<p>' . esc_html__( 'Password protect your web site. Users will be asked to enter a password to view the site.', 'password-protected' );
		echo '<br />' . esc_html__( 'For more information about Password Protected settings, view the "Help" tab at the top of this page.', 'password-protected' );
		echo '</p>';
	...
	public function password_protected_status_field() {
		...
		echo '<label><input name="password_protected_status" id="password_protected_status" type="checkbox" value="1" ';
		echo checked( 1, get_option( 'password_protected_status' ), false ) . ' /> ';
		echo esc_html__( 'Enabled', 'password-protected' ) . '</label>';
	...
	public function password_protected_admin_notices() {
		...
		// Check Support.
		$screens = $this->plugin_screen_ids( [ 'dashboard', 'plugins' ] );
	...
	public function plugin_screen_ids( $screen_id = '' ) {
		...
		$screen_ids = [ 'options-' . $this->options_group, 'settings_page_' . $this->options_group ];
